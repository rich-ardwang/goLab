...
func main() {
	...
	for query := 0; query < maxGoroutines; query++ {
		//每个goroutine需要自己复制一份要查询的副本，
		//不然所有的查询会共享同一个查询变量
		go func(q int) {
			co := performQueries(p)
			performQueries2(q, p, co)
			wg.Done()
		}(query)
	}
...
func performQueries(p *pool.Pool) io.Closer {
	//从池里请求一个连接
	conn, err := p.Acquire()
	if err != nil {
		log.Println(err)
...
func performQueries2(query int, p *pool.Pool, conn io.Closer) {
	//将该连接释放回池里
	defer p.Release(conn, conn.(*dbConnection).ID)
